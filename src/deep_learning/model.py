...
class LaneDetectionModel(nn.Module):
    ...
    def __init__(self, num_classes: int = 2, encoder: str = "resnet34"):
        ...
        if HAS_SMP:
            ...
            self.model = LaneNet(num_classes=num_classes)
...
class LaneDetectionTrainer:
    ...
    def __init__(self, model: nn.Module, device: str = "cuda"):
        ...
        if HAS_SMP and hasattr(smp, 'losses'):
            ...
            self.criterion = nn.CrossEntropyLoss() if getattr(model, 'num_classes', 2) > 1 else nn.BCEWithLogitsLoss()
            self.metrics = []
