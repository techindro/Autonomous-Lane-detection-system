...
class HoughLaneDetector:
    ...
    def calculate_curvature(self, left_line: Optional[np.ndarray], 
        ...
        ploty_left = np.linspace(min(left_line[1], left_line[3]), max(left_line[1], left_line[3]), 20)
        plotx_left = np.linspace(left_line[0], left_line[2], 20)
        
        ploty_right = np.linspace(min(right_line[1], right_line[3]), max(right_line[1], right_line[3]), 20)
        plotx_right = np.linspace(right_line[0], right_line[2], 20)
        
        # Fit second order polynomials
        left_fit = np.polyfit(ploty_left * ym_per_pix, plotx_left * xm_per_pix, 2)
        right_fit = np.polyfit(ploty_right * ym_per_pix, plotx_right * xm_per_pix, 2)
        ...
        left_curvature = ((1 + (2 * left_fit[0] * y_eval + left_fit[1]) ** 2) ** 1.5) / left_denom if left_denom > 1e-8 else 0.0
        right_curvature = ((1 + (2 * right_fit[0] * y_eval + right_fit[1]) ** 2) ** 1.5) / right_denom if right_denom > 1e-8 else 0.0
