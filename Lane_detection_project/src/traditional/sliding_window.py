...
class SlidingWindowDetector:
    ...
    def fit_polynomial(self, binary_warped: np.ndarray, 
        ...
        if len(lefty) > 2 and len(leftx) > 2:
            left_fit = np.polyfit(lefty, leftx, 2)
        else:
            left_fit = np.array([0, 0, binary_warped.shape[1] * 0.25])
            
        if len(righty) > 2 and len(rightx) > 2:
            right_fit = np.polyfit(righty, rightx, 2)
        else:
            right_fit = np.array([0, 0, binary_warped.shape[1] * 0.75])
    ...
    def measure_curvature(self, ploty: np.ndarray, 
        ...
        left_denom = np.absolute(2 * left_fit_cr[0])
        right_denom = np.absolute(2 * right_fit_cr[0])
        
        left_curverad = ((1 + (2 * left_fit_cr[0] * y_eval * ym_per_pix + left_fit_cr[1])**2)**1.5) / left_denom if left_denom > 1e-8 else 0.0
        right_curverad = ((1 + (2 * right_fit_cr[0] * y_eval * ym_per_pix + right_fit_cr[1])**2)**1.5) / right_denom if right_denom > 1e-8 else 0.0
    ...
    def detect(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        # Measure curvature
        left_curverad, right_curverad = self.measure_curvature(ploty, left_fitx, right_fitx)
        curvature = (left_curverad + right_curverad) / 2 if (left_curverad > 0 and right_curverad > 0) else max(left_curverad, right_curverad)
        ...
        visualization = cv2.addWeighted(image, 1.0, newwarp, 0.4, 0)
        
        left_lane = np.array([int(left_fitx[-1]), int(ploty[-1]), int(left_fitx[0]), int(ploty[0])])
        right_lane = np.array([int(right_fitx[-1]), int(ploty[-1]), int(right_fitx[0]), int(ploty[0])])
